...
rl.question(chalk.cyan('Gunakan proxy? (y/n): '), (useProxyAnswer) => {
  if (useProxyAnswer.toLowerCase() === 'y') {
    ...
    console.log(chalk.yellowBright('[→] Proxy not used.'));
  }
      ...
      for (let i = 0; i < count; i++) {
        ...
        if (useProxy) {
          ...
          try {
            ...
            if (publicIP) {
              console.log(chalk.blue(`Use IP: ${publicIP}`));
            } else {
              console.log(chalk.blue(`Use proxy: ${chosenProxy}`));
            }
          } catch (err) {
            console.error(chalk.red(`Error creating proxy agent for ${chosenProxy}: ${err.message}`));
            continue;
          }
        }
